document.addEventListener('DOMContentLoaded', () => {
    ...
    // Header link settings elements
    const headerMailUrlInput = document.getElementById('header-mail-url');
    const headerMailTitleInput = document.getElementById('header-mail-title');
    const headerImageSearchUrlInput = document.getElementById('header-image-search-url');
    const headerImageSearchTitleInput = document.getElementById('header-image-search-title');

    // Header link elements to update
    ...
    const headerMailIcon = document.getElementById('header-mail-icon'); // Assuming you add an img tag for icon
    ...
    const headerImageSearchIcon = document.getElementById('header-image-search-icon'); // Assuming you add an img tag for icon
    ...
    let headerLinks = JSON.parse(localStorage.getItem('headerLinks')) || {
        mail: {
            ...
            title: 'Hotmail',
            icon: '' // Will be generated by getFaviconUrl if empty
        },
        imageSearch: {
    ...
    /**
     * Updates the header links displayed on the page.
     */
    function updateHeaderLinks() {
        // Mail link
        headerMailLink.href = headerLinks.mail.url;
        headerMailLink.setAttribute('aria-label', `Go to ${headerLinks.mail.title}`);
        headerMailText.textContent = headerLinks.mail.title;
        headerMailIcon.src = headerLinks.mail.icon || getFaviconUrl(headerLinks.mail.url);


        // Image Search link
        headerImageSearchLink.href = headerLinks.imageSearch.url;
        headerImageSearchLink.setAttribute('aria-label', `Go to ${headerLinks.imageSearch.title}`);
        headerImageSearchText.textContent = headerLinks.imageSearch.title;
        headerImageSearchIcon.src = headerLinks.imageSearch.icon || getFaviconUrl(headerLinks.imageSearch.url);
    ...
    function exportData() {
        const data = {
            ...
            headerLinks: headerLinks // Include header links in export
        };
    ...
    function importData(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                // Import header links
                if (importedData.headerLinks && typeof importedData.headerLinks === 'object') {
                    headerLinks = importedData.headerLinks;
                } else {
                    console.warn('Header links data missing or corrupted during import.');
                }
                ...
                renderLinks();
                updateHeaderLinks(); // Update header links after import
    ...
    settingsButton.addEventListener('click', () => {
        ...
        languageSelector.value = currentLanguage;
        // Populate header link settings inputs
        headerMailUrlInput.value = headerLinks.mail.url;
        headerMailTitleInput.value = headerLinks.mail.title;
        headerImageSearchUrlInput.value = headerLinks.imageSearch.url;
        headerImageSearchTitleInput.value = headerLinks.imageSearch.title;
    ...
    saveSettingsButton.addEventListener('click', () => {
        ...
        // Save header link settings
        headerLinks.mail.url = headerMailUrlInput.value.trim();
        headerLinks.mail.title = headerMailTitleInput.value.trim();
        headerLinks.imageSearch.url = headerImageSearchUrlInput.value.trim();
        headerLinks.imageSearch.title = headerImageSearchTitleInput.value.trim();
        // Update header link icons as well, if they were custom and now cleared,
        // they will fallback to generated favicons on updateHeaderLinks()
        headerLinks.mail.icon = getFaviconUrl(headerLinks.mail.url);
        headerLinks.imageSearch.icon = getFaviconUrl(headerLinks.imageSearch.url);
        ...
        updateHeaderLinks(); // Update header links on page
        closeSettingsModal();
    });
    ...
    updateHeaderLinks(); // Update header links on page load
});